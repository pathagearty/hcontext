...
def valid_settings(**overrides: object) -> AppSettings:
    values: dict[str, object] = {
        ...
        "foundry_manufacturing_agent_endpoint": (
            ...
            "manufacturing-agent/endpoint/protocols/openai/responses"
        ),
...
class SettingsTests(unittest.TestCase):
    def test_valid_saved_agent_configuration(self) -> None:
        ...
        self.assertEqual(settings.foundry_agent("manufacturing").model, "gpt-5")
    ...
    def test_foundry_values_are_trimmed_and_unknown_role_is_rejected(self) -> None:
        settings = valid_settings(foundry_manufacturing_model="  gpt-5  ")
        self.assertEqual(settings.foundry_manufacturing_model, "gpt-5")
    ...
    def test_agent_id_must_match_endpoint(self) -> None:
        settings = valid_settings(foundry_manufacturing_agent_id="different-agent")
        with self.assertRaises(FoundryConfigurationError) as context:
            settings.require_foundry()
    ...
    def test_agent_endpoints_must_be_distinct(self) -> None:
        settings = valid_settings(
            foundry_hexacontext_agent_id="manufacturing-agent",
            foundry_hexacontext_agent_endpoint=(
                "https://example.services.ai.azure.com/api/projects/demo/agents/"
                "manufacturing-agent/endpoint/protocols/openai/responses"
            ),
        ...
        self.assertTrue(any("agent IDs must be distinct" in issue for issue in issues))
        self.assertTrue(any("agent endpoints must be distinct" in issue for issue in issues))

    def test_agent_endpoint_must_be_https_responses_url(self) -> None:
        settings = valid_settings(
            foundry_manufacturing_agent_endpoint="http://example.test/manufacturing-agent/invoke"
        )
        issues = settings.foundry_configuration_issues()
        self.assertTrue(any("absolute HTTPS" in issue for issue in issues))

        settings = valid_settings(
            foundry_manufacturing_agent_endpoint="https://example.test/manufacturing-agent/invoke"
        )
        issues = settings.foundry_configuration_issues()
    ...
    def test_explicit_env_file_loads_and_environment_wins(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            env_path.write_text(
                "FOUNDRY_API_VERSION=from-file\nFOUNDRY_MANUFACTURING_MODEL=file-model\n",
                encoding="utf-8",
            )
            with patch.dict(os.environ, {"FOUNDRY_MANUFACTURING_MODEL": "environment-model"}):
                settings = AppSettings(_env_file=env_path, _env_file_encoding="utf-8")
        self.assertEqual(settings.foundry_api_version, "from-file")
        self.assertEqual(settings.foundry_manufacturing_model, "environment-model")
